...
1.16KB页的组织结构；每一页包含页目录和用户数据，表头中包含prev以及next指针。两层的B+树可以存2400万行数据，超过以后，树的高度会变成3层，这个时候就不建议去创建索引，而是要进行分表了。

2.回表：创建的其他索引如果需要查询的话，在叶子结点中存着主键索引。走不走联合索引是优化器的决定。

3.最左前缀的原则的由来：索引需要排序。

4.bufferpool默认128M, 有自己的淘汰机制。

5.free链表和flush链表：如果内存页在flush链表上，表示这是一个脏页。更新以后就会加入flush链表。

6.LRU链表：新使用的内存页要放到链表的头部。全表扫描会把之前的热点数据全部覆盖掉。前5/8的LRU链表是热数据，后面是冷数据区域的数据。所以这样全表扫描不会取代掉热点的存在。

7.如果两次查询冷数据区域时，时间差小于1s,表示可能在进行全表扫描，所以不会加入到热数据区域，如果时间差大于1s, 那么表示这可能是一个热数据，所以要加入热数据区。目的是为了阻止全表扫描换血。

8.redo log和bin log; 随机IO太耗费时间，所以redo log可以在mysql挂掉重启以后还能恢复数据。对redo log的读取或者再次从磁盘中读取原页是一个顺序IO。

9.redo log是一个乒乓缓冲区，当需要进行切换的时候，会持久化满的redo log。可以增大缓冲区大小和缓冲区数量。

10.redo log的持久化的时机：三种配置：a:提交事务时，把持久化的任务交给某个后台定时的线程去做；b:提交事务时，立即对redo log进行持久化；c：事务提交时，立即将redo log写入到操作系统的缓冲区，并不会直接对redo log进行持久化，如果数据库挂了，操作系统仍然可以进行redo log的持久化。操作系统有定时的flush操作。

11.binlog不是innoDB中的概念，而是上一层mysql的概念。binlog记录的是局域，redo log记录的是物理地址，记录数据页的物理修改；binlog主要是主从的需要。

12.undo log是一个反向操作的日志。用来提供回滚操作。
...
1.service mysql start/stop/restart,启动mysql服务，/etc/inti.d/mysql start或者 chkconfig mysql on/off;mysql -u jt -p xxxx

2.数据库存放目录：ps -ef|grep mysql, 配置文件/etc/my.cnf中修改字符集UTF-8，然后可以通过命令show variables like '%char%'查看。

3.system clear+ ctrl-L

4.myisam 和innoDB之间的区别：前者是性能优先（表锁），后者是事务优先（行锁）。show engines

5.当前使用的引擎： show variables like '%storage_engine%'

6.创建表的时候：

> create database myDB;
> use myDB;
> create table test(id int(4))primary key(id) ENGINE=MyISAM AUTO_INCREMENT=1 ;

### 2.性能优化

1.优化原因：性能低+执行时间太长+SQL语句不佳（链接表）+索引失效+服务器参数设置不好（缓冲区、线程数）等。

2.SQL语句：

> 编写过程：select distinct ...from ... join...on...where...group by...having...ordered by...limit...
>
> 解析过程：from... on...join...where...group by...having...select distinct...ordered by...limit...

3.索引是一个帮助高效获取数据的数据结构

> 索引的弊端：
>
> - 本身很大，需要存储空间
> - 索引不适用于以下情况：数据量少+频繁更新的字段+很少使用的字段
> - 索引提高了查找的效率，但是降低了增删改的效率
>
> 索引的优点：
>
> - 提高查询效率（降低IO使用率）
...
4.B数的三层结构可以存放上百万条数据（2000万）,但是不能进行范围查询。

### 3.索引

1.分类：单值索引+唯一索引（不能重复）+复合索引（多个列构成的索引）,主键索引与唯一索引非常相似，但是不能为null；

2.创建索引：create 索引类型 索引名 on 表名（某个字段列）、alter table 表名 索引类型 索引名（字段）

> 单值索引：create index index1 on stu(age);       alter table stu add index index1(stu);
>
> 唯一索引：create unique index index2 on stu(id); alter table stu add unique index index2(stu);
>
> 复合索引：create index index3 on stu(name, age); alter table stu add index index3(name, age);

3.删除索引：drop index1 on stu;

4.查看索引：show index from stu;

5.分析sql的执行计划：explain 可以模拟优化器执行sql语句。

6.MySQL的自动查询优化可能会干扰我们的优化（小表驱动大表）。

## 4.优化

1.extra字段

> using temporary=group by,性能损耗大，用到了临时表。
>
> using filesort=order by，性能损耗大，需要额外进行一次排序或者查询。
>
> using index=性能提升，索引覆盖，此次查询不读取源文件（不需要回表查询）。索引覆盖时，会对possible_key和key产生影响：
>
> 在查询语句中，如没有where分句，那么索引只出现在key中，有where分句，那么索引才会同时出现在possble_key和key中。
>
> using where=需要回表查询：要查询的列并没有被索引覆盖，只能回原表查询。具体过程是，服务器在存储引擎检索行后再进行过滤，就是先读取整行数据，再按照where的条件进行检查，符合就留下，不符合就丢弃。
>
> impossible where=where子句永远为false。
>
> using join buffer:使用了连接缓存。

2.查询的时候顺序应当要和索引的列的顺序一致，如果跨列会导致后面的列索引失效（索引退化）；

3.如果查询语句中，where 和 order by语句是符合索引的顺序，就不会使用using filesort。最左原则，如果查询与索引的顺序完全一致（不跨列）则全部使用，如果部分一致，则使用部分索引。

4.单表优化：添加合适的索引+及时删除不必要的索引防止干扰+合适的语句组织（范围查询in有可能会失效，多个查询的时候可以把它放到后边去）

5.双表优化：小表驱动大表（小表.x=大表.x）会减少在两个表中的反复横跳。也就是说双层循环中，外循环应该使用小的数。 对于主表而言，由于会被反复用于跟副表中的数据比较，所以要将主表的经常使用的列建立索引。

6.多表查询：跟上述差不多。

7.避免索引失效的原则：复合索引不要跨列或者无序使用（最佳左前缀）+尽量使用全索引匹配+不要在索引上进行任何数据操作（四则运算、函、类型转换）都会导致索引失效where a.x+4=y；对于复合索引，如果左边失效，那么右边全部失效。此外复合索引也不能使用不等于或者is null等逻辑操作。

8.避免索引失效的原则：索引优化由于优化区的存在,可能对于小部分比如范围查询使得索引失效而不会被优化到。我们可以尽量使用索引覆盖来规避这个概率问题+like模糊查询尽量以常量开头，不要用'%x%'这样的的形式，如果必须使用%%模糊查询，可以使用索引覆盖一定程度的挽救。

9.尽量不要使用类型转换，否则索引失效：

> select * from teacher where tname='123'VS select * from teahcer where tname=123;

10.尽量不要使用or，否则导致索引失效；or甚至可以把左侧的索引也搞失效，因为or的原理是左右两边的条件满足一个就可以。

11.其他的一些优化方法

> - exists和in  exists将主查询的结果放到子查询中进行校验，如果有数据那么校验成功。
>
>   - select tname from teacher where exists (select * from teacher) 
>
>   - 如果主查询的数据集大，那么使用in,一般效率更高。
>   - 如果子查询的数据集大，那么使用exists
>
> - order by,using filesort
>
>   - 双路排序：第一次查询的是排序字段后进行排序，在缓冲区中进行，第二次扫描其他字段（4.1版本之前）；两次IO消耗性能。
>   - 单路排序：读取一次全部字段，在buffer中跳出排序字段进行排序，但是不一定是单次IO，还有可能是多次IO。如果数据量特别大，那么会进行分片，性能进一步降低。如果buffer太低会把排序策略改成双路。
>
> - 避免使用select *；
>
> - 排序时保证排序的一致性，都是升序或者降序。

12.SQL排查，慢查询日志，用于记录mysql中响应时间超过阈值的SQL语句。该日志默认是关闭的，开发时可以打开，最终部署时关闭。默认是10秒。可以设置临时与永久设置阈值以及是否要开启慢查询日志。

13.分析海量数据，分析每个语句的执行时间，可以打开profiles，set profiling on; 然后show Profiles；或者show profile all for query，每一个语句都有自己的ID。

14.看全局的查询日志，记录开启之后的全部SQL语句，这些全局的记录操作比较耗费性能，正式部署时需要关闭。

15.锁机制：解决因为资源共享而造成的并发问题。show open tables;

> - 按照操作类型：读锁（共享锁）+写锁（互斥锁）
>
> - 按照操作范围：行锁+表锁+页锁
>
>   表锁：开销小，加锁快，无死锁；但是并发度低，容易造成锁冲突；
>
>   行锁：开销大，加锁慢，容易死锁，但是不容易冲突，并发度高；

16.mysql表级锁的锁模式：

> myisam在执行select查询之前会给涉及的所有表加读锁，在执行其他写操作时，会自动给涉及的表加写锁。
>
> 所以：当表被读锁lock时，只阻塞其他线程的写请求；当表被加写锁时，其他表无法读写。

17.行锁的注意事项：

> - 如果没有索引那么行锁会转变为表锁。
> - 行锁的一种特殊情况：间隙锁：值在范围内，但是却不存在：在where中没有id在某个范围内的数据，mysql会自动给间隙加锁（行锁）。如果有where，那么加锁的范围就是where的范围，而并不是实际数据的范围。
> - 在查询的时候可以在句尾加一句for update，从而对查询的数据进行加锁；

18.研究学习的时候可以：

> set autocommit=0；  start transaction;  begin;从而关闭自动提交。

19.mysql自带的cache大小：set global query_cache_size=xxxx;
...
![img](https://pic2.zhimg.com/v2-9364243f72f1016d5f5c193518078e35_b.jpg)
...
![img](https://pic2.zhimg.com/v2-3b2fede7537d0723c4545b488a28728d_b.jpg)
...
![img](https://pic4.zhimg.com/v2-810dd67ef648e959502a9a9314c79cf7_b.jpg)
...
![img](https://pic4.zhimg.com/v2-cf033f1c2ccb9460461ae9ca21045467_b.jpg)

![img](https://pic4.zhimg.com/v2-c7812473df48c94422fdf47a99205c4b_b.jpg)

![img](https://pic1.zhimg.com/v2-e74be2b9829f35769c1dc3af0bd1a828_b.jpg)
...
- 除了数据本身之外，数据库还维护一个满足特定查找算法的数据结构，这些数据结构以某种方式引用（指向）数据，这样就可以在这些数据结构上实现高级查找算法。这种数据结构，就是索引。
...
非聚簇索引=索引与数据分开存放，索引的叶子结点指向了数据的对应行。在聚簇索引上创建的索引都是辅助索引。
...
![img](https://pic3.zhimg.com/v2-bf07856ffbdfe61816d1ecdd4ec583a6_b.jpg)
...
![img](https://pic3.zhimg.com/v2-e213b1289dd51d5943d1c5305ab5b63e_b.jpg)
...
![img](https://pic3.zhimg.com/v2-7281aac18de64960d3b062a8c483117a_b.jpg)
...
![img](https://pic2.zhimg.com/v2-7d335799db4209b7fe692bcbc991453d_b.jpg)
...
![img](https://pic3.zhimg.com/v2-c3392c098bd23a1f4a2db4063a509bb2_b.jpg)
...
- 主要就是通过Hash算法（常见的Hash算法有直接定址法、平方取中法、折叠法、除数取余法、随机数法），将数据库字段数据转换成定长的Hash值，与这条数据的行指针一并存入H	ash表的对应位置；如果发生Hash碰撞（两个不同关键字的Hash值相同），则在对应Hash键下以链表形式存储。
...
![img](https://pic2.zhimg.com/v2-f0c806d32a0f2e131161c94f95cb8815_b.jpg)
...
![img](https://pic4.zhimg.com/v2-d6c73b9ba2715d603825fec1201abd2f_b.jpg)
...
![img](https://pic2.zhimg.com/v2-de3b7d7d767eaf51aa18e5983cf4b261_b.jpg)
...
![img](https://pic3.zhimg.com/v2-3352081d00007185fc57823996fd6d56_b.jpg)
...
![img](https://pic1.zhimg.com/v2-9ff2debf307f7640a7bbe56383a4b970_b.jpg)
...
![img](https://pic1.zhimg.com/v2-603615aac80651f383814e5ad8ef96d4_b.jpg)
...
![img](https://pic2.zhimg.com/v2-6a3eab48d4518a149e6ddb36c8928b39_b.png)
...
![img](https://pic4.zhimg.com/v2-7593b6529fd34ea65ce2fd7b6dea438b_b.png)
...
![img](https://pic1.zhimg.com/v2-24448ff16e81b63a72b922e1bcaf8b8c_b.jpg)
...
![img](https://pic2.zhimg.com/v2-a8dc2245af5048ac07de40e63afaf6dd_b.jpg)
...
![img](https://pic2.zhimg.com/v2-3a3ded8492b4f2fa129a088865e007e1_b.jpg)
